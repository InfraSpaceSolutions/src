...
@interface StatTipListViewController ()
...
@implementation StatTipListViewController
...
- (void)viewDidLoad
{
    ...
    [[UTCAPIClient sharedClient] getStatTipFor:[loc busID] withBlock:^(NSArray* attr, NSError *error) {
        if (error) {
            NSLog(@"tip stats load error at view controller");
...
-(UITableViewCell*) tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [[cell summaryLabel] setText:[rec objectForKey:@"TipText"]];
    [[cell alternateLabel] setText:@"REVIEW"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    NSUInteger row = [indexPath row];
    NSDictionary* rec = [stats objectAtIndex:row];
    [[UTCApp sharedInstance] setSelectedReview:rec];
    [[[UTCApp sharedInstance] rootViewController] openBusinessTipViewController];
