//
//  StatCheckInListViewController.m
//  unitethiscity
//
...
@interface StatCheckInListViewController ()

@property (strong, nonatomic) NSArray* stats;
@property (strong, nonatomic) LocationInfo* loc;
@end

@implementation StatCheckInListViewController
...
- (void)viewDidLoad
{
    ...
    [self setScreenName:@"Stat CheckIns"];
    ...
    [[UTCAPIClient sharedClient] getStatCheckInFor:[loc busID] andUser:[[UTCApp sharedInstance] selectedAccount] andScope:([[UTCApp sharedInstance] selectedRange]+1) withBlock:^(NSArray* attr, NSError *error) {
        if (error) {
            NSLog(@"check in stats load error at view controller");
...
-(UITableViewCell*) tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [[cell alternateLabel] setText:@"1 POINT"];
