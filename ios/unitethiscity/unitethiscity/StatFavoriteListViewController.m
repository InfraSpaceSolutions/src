...
@interface StatFavoriteListViewController ()

@property (strong, nonatomic) NSArray* stats;
@property (strong, nonatomic) LocationInfo* loc;
@end

@implementation StatFavoriteListViewController
...
- (void)viewDidLoad
{
    ...
    [self setScreenName:@"Stat Favorites"];
    ...
    [[UTCAPIClient sharedClient] getStatFavoriteFor:[loc busID] withBlock:^(NSArray* attr, NSError *error) {
        if (error) {
            NSLog(@"favorite stats load error at view controller");
...
-(UITableViewCell*) tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [[cell alternateLabel] setText:@"FAVORITE"];
