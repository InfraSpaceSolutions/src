//
//  SummaryCheckInListViewController.m
//  unitethiscity
//
...
@interface SummaryCheckInListViewController ()

@property (strong, nonatomic) NSArray* stats;
@property (strong, nonatomic) LocationInfo* loc;
@end

@implementation SummaryCheckInListViewController
...
- (void)viewDidLoad
{
    ...
    [self setScreenName:@"Summary CheckIn"];
    ...
    [[UTCAPIClient sharedClient] getStatSummaryCheckInsFor:[loc busID] andScope:([[UTCApp sharedInstance] selectedRange] +1) withBlock:^(NSArray* attr, NSError *error) {
        if (error) {
            NSLog(@"checkin summary load error at view controller");
...
-(UITableViewCell*) tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString* summaryText = [NSString stringWithFormat:@"%@",[rec objectForKey:@"Count"]];
