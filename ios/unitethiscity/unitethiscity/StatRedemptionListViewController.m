//
//  StatRedemptionListViewController.m
//  unitethiscity
//
...
@interface StatRedemptionListViewController ()

@property (strong, nonatomic) NSArray* stats;

@end

@implementation StatRedemptionListViewController
...
- (void)viewDidLoad
{
    ...
    [self setScreenName:@"Stat Redemption"];
    ...
    [[UTCAPIClient sharedClient] getStatRedemptionFor:[loc busID] andUser:[[UTCApp sharedInstance] selectedAccount] andScope:([[UTCApp sharedInstance] selectedRange]+1) withBlock:^(NSArray* attr, NSError *error) {
        if (error) {
            NSLog(@"redemption stats load error at view controller");
...
-(UITableViewCell*) tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [[cell summaryLabel] setText:[rec objectForKey:@"Deal"]];
    double dealAmount = [[rec objectForKey:@"Amount"] doubleValue];
    [[cell alternateLabel] setText:[LocationContext formatDeal:dealAmount]];
    [[cell optionalLabel] setText:[rec objectForKey:@"Pin"]];
    [cell.locPicture setImageWithURL:[NSURL URLWithString:[LocationInfo businessImageURIFromGuid:[rec objectForKey:@"BusGuid"]]]
