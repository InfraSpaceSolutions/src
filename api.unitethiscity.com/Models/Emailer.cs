 ...
 * 
 * Description:
 * Class to send electronic mail using database managed credentials
******************************************************************************/
using System;
using System.Data;
...
namespace com.unitethiscity.api.Models
{
    public class EMailer
    {
        ...
        public EMailer()
        {
            ...
            // Set up the email server credential variables from the database
            EMailServer = SiteSettings.GetValue("EMailServer");
            EMailAccount = SiteSettings.GetValue("EMailAccount");
            EMailPassword = SiteSettings.GetValue("EMailPassword");
            EMailSender = SiteSettings.GetValue("EMailSender");
            EMailFrom = SiteSettings.GetValue("EMailFrom");
            EMailFromDisplayName = SiteSettings.GetValue("EMailFromDisplayName");
            EMailReplyTo = SiteSettings.GetValue("EMailReplyTo");
            EMailNotify = SiteSettings.GetValue("EMailNotify");
            EMailServerPort = WebConvert.ToInt32(SiteSettings.GetValue("EMailServerPort"), 25);
        }
        /// <summary>
        ...
        public void SendStandard(string emlSubject, string emlBody, string emlAltBody, string emlRecipients, string emlReplyTo, bool emlIsHtml)
        {
        ...
        void SMTP_Send(MailMessage message, string recipients)
        {
