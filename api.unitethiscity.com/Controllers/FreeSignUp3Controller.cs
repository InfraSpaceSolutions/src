/******************************************************************************
 * Filename: FreeSignUp3Controller.cs
 * Project:  UTC WebAPI
 * 
 * Description:
...
{
    public class FreeSignUp3Controller : ApiController
    {
        WebDBContext db;
        ...
        public LoginCredentials Post(FreeSignUp3 jn)
        {
            db = new WebDBContext();
            ...
            jn.AccZip = CleanOptionalField("Zip", jn.AccZip, 10);

            jn.AccBirthdate = CleanOptionalField("Birth Date", jn.AccBirthdate, 50);
            // force a valid/supported gender string
            jn.AccGender = CleanOptionalField("Gender", jn.AccGender, 20);
            ...
            // clean up the facebook identifier
            jn.AccFacebookIdentifier = CleanOptionalField("Facebook Identifier", jn.AccFacebookIdentifier, 256);
            ...
            // make sure the facebook identifier isn't already in use
            if (jn.AccFacebookIdentifier.Length > 0)
            {
                if (db.TblAccounts.Count(target => target.AccFacebookIdentifier == jn.AccFacebookIdentifier) > 0)
                {
                    throw new HttpResponseException(Request.CreateErrorResponse(HttpStatusCode.BadRequest, "Facebook identifier already in use."));
                }
            }
            ...
            if ((age < 18) || (age > 100))
            {
                // We no longer require a valid birthdate or 18 years of age
            ...
            rsAcc.AccZip = jn.AccZip;

            rsAcc.AccFacebookIdentifier = jn.AccFacebookIdentifier;
            ...
            Logger.LogAction("SignUp-3", rsAcc.AccID, 0);
        ...
        /// <param name="value">value to test</param>
        /// <param name="maxlen">truncate to this length</param>
        protected string CleanOptionalField(string name, string value, int maxlen)
        {
            string ret = WebConvert.ToString(value, "").Trim();
            return (ret.Length > maxlen) ? ret.Substring(0, maxlen) : ret;
        }
        ...
        /// </summary>
        /// <param name="jn">JoinNow properties</param>
        protected void NotifyAdministrator(FreeSignUp2 jn)
        {
            StringBuilder msg = new StringBuilder();
            ...
            try
            {
                ...
                msg.AppendLine("Account Name: " + jn.AccFName + " " + jn.AccLName);
                msg.AppendLine("Account EMail: " + jn.AccEMail);
                msg.AppendLine("Account Zip: " + jn.AccZip);
                msg.AppendLine(new string('=', 60));
                msg.AppendLine("Account Gender: " + jn.AccGender);
                msg.AppendLine("Account Date of Birth: " + jn.AccBirthdate);
                msg.AppendLine(new string('=', 60));
        ...
        /// </summary>
        /// <param name="jn">JoinNow properties</param>
        protected void NotifyAdministrator(FreeSignUp3 jn)
        {
            StringBuilder msg = new StringBuilder();
            ...
            try
            {
                ...
                msg.AppendLine("Account Gender: " + jn.AccGender);
                msg.AppendLine("Account Date of Birth: " + jn.AccBirthdate);
                msg.AppendLine("Account Facebook Identifier: " + jn.AccFacebookIdentifier);
                msg.AppendLine(new string('=', 60));
                msg.AppendLine("Promo Code: " + jn.PromoCode);
                msg.AppendLine("Source: Mobile App");
                EMailer email = new EMailer();
